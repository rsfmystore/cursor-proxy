  ...
  try {
    ...
    if (process.env.TOKENS) {
      ...
      if (envTokens.length > 0) {
        localTokens = envTokens;
        console.log(`Loaded ${localTokens.length} tokens from environment`);
        return true;
      }
    }
    ...
    if (fs.existsSync(tokenPath)) {
      ...
      if (content) {
        localTokens = content.split(',')
          .map(t => t.trim())
          .filter(t => t.length > 0);
        console.log(`Loaded ${localTokens.length} tokens from file`);
        return true;
      }
    }
    console.error('No tokens found in environment or token.txt');
    return false;
  } catch (error) {
